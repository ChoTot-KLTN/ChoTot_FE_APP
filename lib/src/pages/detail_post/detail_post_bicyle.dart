...
import 'package:chotot_app/src/models/post/bicyle_model.dart';
...
class DetailPostBicyle extends StatefulWidget {
  const DetailPostBicyle({Key? key, this.postdetail}) : super(key: key);
  final PostModel? postdetail;
  @override
  State<DetailPostBicyle> createState() => _DetailPostBicyleState();
}

class _DetailPostBicyleState extends State<DetailPostBicyle> {
  int currentIndex = 0;
  List<String>? listImgs = []; // list Img use to Slider
  PageController pageController = PageController(initialPage: 0);
  String avatar = "a";
  PostBycleModel? postBicyleModel;
  bool isLoading = true;
  ...
  getPostDetail() async {
    try {
      ...
      if (result.statusCode == 200) {
        ...
        var jsonResponse = PostBycleModel.fromMap(data);
        setState(() {
          postBicyleModel = jsonResponse;
          isLoading = false;
        });
      } else {
        print("Lỗi 404 ");
  ...
  getListRecommend() async {
    var listPost = await PostServiceRepository()
        .getAllPostWithType(page: 0, limit: 10, status: 2, type: "PostBicycle");
                ...
                : Column(
                    children: [
                      buildDetail(Icons.payment_outlined,
                          "Trạng thái: ${postBicyleModel!.type}"),
                      buildDetail(
                          Icons.flag, "Hãng: ${postBicyleModel!.brand}"),
                      buildDetail(Icons.category_outlined,
                          "Thể loại: ${postBicyleModel!.typeBicycle}"),
                      buildDetail(Icons.engineering_outlined,
                          "Công suất: ${postBicyleModel!.engine}"),
                      buildDetail(Icons.history_edu,
                          "Tình trạng: ${postBicyleModel!.statusBicycle}"),
                      buildDetail(Icons.high_quality_outlined,
                          "Bảo hành: ${postBicyleModel!.guarantee}"),
                    ],
                  ),
          ),
          SizedBox(
            ...
            child: SizedBox(
              ...
              child: ListView(
                ...
                children: [
                  ...
                  buildItemChat("Giá xe có thể thương lượng được không ạ?"),
                  SizedBox(
                    width: 5,
                  ),
                  buildItemChat("Xe có còn bảo hành không ạ?"),
                ],
              ),
            ),
          ),
              ...
              children: [
                ...
                Expanded(
                  child: isLoading
                      ? Text("Địa chỉ")
                      : Text(
                          postBicyleModel!.address.detail +
                              ", " +
                              postBicyleModel!.address.village +
                              ", " +
                              postBicyleModel!.address.district +
                              ", " +
                              postBicyleModel!.address.province,
                          maxLines: 2,
                          overflow: TextOverflow.ellipsis,
                        ),
                ),
