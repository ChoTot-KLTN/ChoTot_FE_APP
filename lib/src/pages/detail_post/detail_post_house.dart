...
import 'package:chotot_app/src/models/post/house_model.dart';
...
class DetailPostHouse extends StatefulWidget {
  const DetailPostHouse({Key? key, this.postdetail}) : super(key: key);
  final PostModel? postdetail;

  @override
  State<DetailPostHouse> createState() => _DetailPostHouseState();
}

class _DetailPostHouseState extends State<DetailPostHouse> {
  int currentIndex = 0;
  List<String>? listImgs = []; // list Img use to Slider
  PageController pageController = PageController(initialPage: 0);
  String avatar = "a";
  HouseModel? postHouseModel;
  bool isLoading = true;
  ...
  getPostDetail() async {
    try {
      ...
      if (result.statusCode == 200) {
        ...
        var jsonResponse = HouseModel.fromMap(data);
        setState(() {
          postHouseModel = jsonResponse;
          isLoading = false;
        });
      } else {
        print("Lỗi 404 ");
  ...
  getListRecommend() async {
    var listPost = await PostServiceRepository()
        .getAllPostWithType(page: 0, limit: 10, status: 2, type: "PostHouse");
                ...
                Row(
                  ...
                  children: [
                    Column(
                      ...
                      children: [
                        Text(
                            (widget.postdetail!.prePrice.toString()).toVND() +
                                " /Tháng",
                            style: TextStyle(
                ...
                : Column(
                    children: [
                      buildDetail(Icons.payment_outlined,
                          "Trạng thái: ${postHouseModel!.type}"),
                      buildDetail(
                          Icons.grass, "Diện tích: ${postHouseModel!.area}"),
                      buildDetail(Icons.bedroom_parent_outlined,
                          "Số phòng ngủ: ${postHouseModel!.numberOfBedroom}"),
                      buildDetail(Icons.bathroom_outlined,
                          "Số phòng tắm: ${postHouseModel!.numberOfBathroom}"),
                      buildDetail(Icons.category_outlined,
                          "Thể loại: ${postHouseModel!.typeHouse}"),
                      buildDetail(Icons.rule_outlined,
                          "Pháp lý: ${postHouseModel!.juridical}"),
                      buildDetail(Icons.chair_rounded,
                          "Tình trạng nội thất: ${postHouseModel!.interiorCondition}"),
                      buildDetail(Icons.height_outlined,
                          "Chiều ngang: ${postHouseModel!.width}"),
                      buildDetail(Icons.height_outlined,
                          "Chiều dài: ${postHouseModel!.height}"),
                    ],
                  ),
          ),
          SizedBox(
          ...
          Padding(
            ...
            child: SizedBox(
              ...
              child: ListView(
                ...
                shrinkWrap: true,
                children: [
                  buildItemChat("Nhà này còn cho thuê không ạ?"),
                  SizedBox(
                    width: 5,
                  ),
                  buildItemChat("Có sẵn nội thất không"),
                  SizedBox(
                    width: 5,
                  ),
                  buildItemChat("Tiền cọc nhà là bao nhiêu"),
                ],
              ),
            ),
          ),
              ...
              children: [
                ...
                Expanded(
                  child: isLoading
                      ? Text("Địa chỉ")
                      : Text(
                          postHouseModel!.address.detail +
                              ", " +
                              postHouseModel!.address.village +
                              ", " +
                              postHouseModel!.address.district +
                              ", " +
                              postHouseModel!.address.province,
                          maxLines: 2,
                          overflow: TextOverflow.ellipsis,
                        ),
                ),
