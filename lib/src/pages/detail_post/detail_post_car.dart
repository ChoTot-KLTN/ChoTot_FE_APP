...
class DetailPostCarScreen extends StatefulWidget {
  ...
  @override
  State<DetailPostCarScreen> createState() => _DetailPostCarScreenState();
}

class _DetailPostCarScreenState extends State<DetailPostCarScreen> {
  ...
  getPostDetail() async {
    try {
      ...
      if (result.statusCode == 200) {
        ...
        var jsonResponse = PostCarModel.fromMap(data);
        setState(() {
          postCarModel = jsonResponse;
          isLoading = false;
        });
      } else {
        print("Lỗi 404 ");
                ...
                : Column(
                    children: [
                      buildDetail(Icons.payment_outlined,
                          "Trạng thái: ${postCarModel!.type}"),
                      buildDetail(Icons.flag, "Hãng: ${postCarModel!.brand}"),
                      buildDetail(Icons.calendar_view_month,
                          "Năm sản xuất: ${postCarModel!.yearOfManufacture}"),
                      buildDetail(Icons.edit_road,
                          "Số km đã đi: ${postCarModel!.numberOfKm}"),
                      buildDetail(Icons.history_edu,
                          "Tình trạng: ${postCarModel!.statusCar}"),
                      buildDetail(Icons.control_point,
                          "Hộp số: ${postCarModel!.carGearbox}"),
                      buildDetail(Icons.fire_extinguisher_outlined,
                          "Nhiên liệu: ${postCarModel!.fuel}"),
                      buildDetail(Icons.event_seat,
                          "Số chỗ: ${postCarModel!.numberOfSeat}"),
                      buildDetail(Icons.color_lens_outlined,
                          "Màu sắc: ${postCarModel!.color}"),
                    ],
                  ),
          ),
          SizedBox(
            ...
            child: SizedBox(
              ...
              child: ListView(
                ...
                children: [
                  ...
                  buildItemChat("Xe chính chủ hay được ủy quyền ạ?"),
                  SizedBox(
                    width: 5,
                  ),
                  ...
                  buildItemChat("Xe có bảo hiểm không ạ?"),
                  SizedBox(
                    width: 5,
                  ),
              ...
              children: [
                ...
                Expanded(
                  child: isLoading
                      ? Text("Địa chỉ")
                      : Text(
                          postCarModel!.address.detail +
                              ", " +
                              postCarModel!.address.village +
                              ", " +
                              postCarModel!.address.district +
                              ", " +
                              postCarModel!.address.province,
                          maxLines: 2,
                          overflow: TextOverflow.ellipsis,
                        ),
                ),
