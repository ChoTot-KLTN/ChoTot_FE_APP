...
import 'package:chotot_app/src/models/post/office_model.dart';
...
class DetailPostOffice extends StatefulWidget {
  ...
  @override
  State<DetailPostOffice> createState() => _DetailPostOfficeState();
}

class _DetailPostOfficeState extends State<DetailPostOffice> {
  int currentIndex = 0;
  ...
  getPostDetail() async {
    try {
      ...
      if (result.statusCode == 200) {
        ...
        var jsonResponse = PostOfficeModel.fromMap(data);
        setState(() {
          postOfficeModel = jsonResponse;
          isLoading = false;
        });
      } else {
        print("Lỗi 404 ");
  ...
  getListRecommend() async {
    var listPost = await PostServiceRepository()
        .getAllPostWithType(page: 0, limit: 10, status: 2, type: "PostOffice");
                ...
                : Column(
                    children: [
                      buildDetail(Icons.payment_outlined,
                          "Trạng thái: ${postOfficeModel!.type}"),
                      buildDetail(
                          Icons.grass, "Diện tích: ${postOfficeModel!.area}"),
                      buildDetail(Icons.directions,
                          "Hướng cửa chính: ${postOfficeModel!.doorDirection}"),
                      buildDetail(Icons.category_outlined,
                          "Loại hình văn phòng: ${postOfficeModel!.typeOffice}"),
                      buildDetail(Icons.rule_outlined,
                          "Pháp lý: ${postOfficeModel!.juridical}"),
                      buildDetail(Icons.chair_rounded,
                          "Tình trạng nội thất: ${postOfficeModel!.interiorCondition}"),
                    ],
                  ),
          ),
          SizedBox(
          ...
          Padding(
            ...
            child: SizedBox(
              ...
              child: ListView(
                ...
                shrinkWrap: true,
                children: [
                  buildItemChat("Mặt bằng này còn cho thuê không ạ?"),
                  SizedBox(
                    width: 5,
                  ),
                  buildItemChat("Tình trạng nội thất như thế nào?"),
                  SizedBox(
                    width: 5,
                  ),
                  buildItemChat("Tiền cọc là bao nhiêu?"),
                  SizedBox(
                    width: 5,
                  ),
                  buildItemChat("Mặt bằng có gần đường lớn không?"),
                ],
              ),
            ),
          ),
              ...
              children: [
                ...
                Expanded(
                  child: isLoading
                      ? Text("Địa chỉ")
                      : Text(
                          postOfficeModel!.address.detail +
                              ", " +
                              postOfficeModel!.address.village +
                              ", " +
                              postOfficeModel!.address.district +
                              ", " +
                              postOfficeModel!.address.province,
                          maxLines: 2,
                          overflow: TextOverflow.ellipsis,
                        ),
                ),
