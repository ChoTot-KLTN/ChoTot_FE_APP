...
import 'package:chotot_app/src/models/post/laptop_model.dart';
...
class DetailPostlaptop extends StatefulWidget {
  const DetailPostlaptop({Key? key, this.postdetail}) : super(key: key);
  final PostModel? postdetail;

  @override
  State<DetailPostlaptop> createState() => _DetailPostlaptopState();
}

class _DetailPostlaptopState extends State<DetailPostlaptop> {
  int currentIndex = 0;
  List<String>? listImgs = []; // list Img use to Slider
  PageController pageController = PageController(initialPage: 0);
  String avatar = "a";
  PostLaptopModel? postMotelRoomModel;
  bool isLoading = true;
  ...
  getPostDetail() async {
    try {
      ...
      if (result.statusCode == 200) {
        ...
        var jsonResponse = PostLaptopModel.fromMap(data);
        setState(() {
  ...
  getListRecommend() async {
    var listPost = await PostServiceRepository()
        .getAllPostWithType(page: 0, limit: 10, status: 2, type: "PostLaptop");
    if (listPost.length == 0) {
      ...
      streamController.sink.add(listPostRecommend);
    }
  }
        ...
        children: [
          ...
          Padding(
            ...
            child: isLoading
                ...
                : Column(
                    children: [
                      buildDetail(Icons.payment_outlined, "Trạng thái: sale"),
                      buildDetail(
                          Icons.flag, "Hãng: ${postMotelRoomModel!.brand}"),
                      buildDetail(Icons.microwave_outlined,
                          "Chip: ${postMotelRoomModel!.microprocessor}"),
                      buildDetail(
                          Icons.sim_card, "Ram: ${postMotelRoomModel!.ram}"),
                      buildDetail(Icons.storage_outlined,
                          "Ổ cứng + ${postMotelRoomModel!.typeHardDrive}: ${postMotelRoomModel!.hardDrive}"),
                      buildDetail(Icons.card_membership_outlined,
                          "Card màn hình: ${postMotelRoomModel!.graphicsCard}"),
                      buildDetail(Icons.history_edu,
                          "Trạng thái: ${postMotelRoomModel!.statusLaptop}"),
                      buildDetail(Icons.rule_outlined,
                          "Bảo hành: ${postMotelRoomModel!.guarantee}"),
                      buildDetail(Icons.color_lens_outlined,
                          "Màu sắc: ${postMotelRoomModel!.color}"),
                    ],
                  ),
          ),
          SizedBox(
          ...
          Padding(
            ...
            child: SizedBox(
              ...
              child: ListView(
                ...
                shrinkWrap: true,
                children: [
                  buildItemChat("Laptop này còn không ạ?"),
                  SizedBox(
                    width: 5,
                  ),
                  buildItemChat("Laptop còn bảo hành không"),
                  SizedBox(
                    width: 5,
                  ),
                  buildItemChat("Laptop có thay thế linh kiện gì chưa"),
                  SizedBox(
                    width: 5,
                  ),
                  buildItemChat("Tình trạng màn hình, pin như thế nào"),
                ],
              ),
            ),
          ),
